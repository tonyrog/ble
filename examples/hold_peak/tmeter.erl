...
-define(HP_90EPD_DOT(I),
    case (I) of
	2 -> (1 bsl ?HP_90EPD_DOT1);
	3 -> (1 bsl ?HP_90EPD_DOT2);
	4 -> (1 bsl ?HP_90EPD_DOT3)
    end).
...
encode(Value, Opts) when is_number(Value), is_map(Opts) ->
    ...
    Frac = round((Value1 - Int) * 10000),  %% 4 digits
    ...
    E = encode_hp_90epd_num(Num),
    << <<0:4, B:4>> || 
	<<B:4>> <= << (F + (1 bsl Ubit) + (1 bsl Sbit) + E):56>> >>.
...
encode_hp_90epd_num(Num) ->
    encode_hp_90epd_num(Num,1).
