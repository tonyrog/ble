...
-define(SCAN_INTERVAL(Tms), ((((Tms)*1000)) div 625)).
-define(CONN_INTERVAL(Tms), ((((Tms)*1000)) div 1250)).

create_connection(Hci, PeerAddr, Options) ->
    %% Scan interval/window for connection (in 0.625ms units)
    ScanInterval = ?SCAN_INTERVAL(?SCAN_INTERVAL_MS),
    ScanWindow   = ?SCAN_INTERVAL(?SCAN_WINDOW_MS),
    ...
    PeerAddrBin = ?ADDR_TO_BIN(PeerAddr),
    ...
    MinInterval = ?CONN_INTERVAL(MinIntervalMs),
    MaxInterval = ?CONN_INTERVAL(MaxIntervalMs),
